...
const NAV_ITEMS: NavItem[] = [
  ...
  { label: "SERVICES", href: "/services" },
  {
    ...
    href: "/our-companies",
    children: [
      { label: "Gulf Resources Facility Management", href: "/our-companies/c1" },
      { label: "Gulf Track Business Setup & Visa Works", href: "/our-companies/c2" },
...
const MobileNav = ({ items, currentPath, closeMenu }:MobileNavProps) => {
    ...
    const getParentLinkClasses = (href: string) => {
        const isActiveParent = currentPath === href || currentPath.startsWith(href + '/');
                        ...
                        <>
                            ...
                            <div 
                                className={`overflow-hidden transition-all duration-300 
                                    ${openIndex === index ? 'max-h-96 py-1' : 'max-h-0'}`}
                            >
...
const NavLinkWithDropdown = ({ item, currentPath }:NavLinkWithDropdownProps) => {
    // Check if the current path exactly matches the item or starts with the item's base path
    const isActiveParent = currentPath === item.href || currentPath.startsWith(item.href + '/');
        ...
        <div 
          className={`absolute top-full left-1/2 -translate-x-1/2 mt-0.5 
                     transition-all duration-300 origin-top ${
                        isDropdownOpen ? 'opacity-100 visible scale-y-100' : 'opacity-0 invisible scale-y-0'
                     } 
                     min-w-[200px]`}
        >
        ...
        <div className="flex gap-4 py-2 items-center">
          ...
          <div className="hidden md:flex gap-4 h-full items-center">
            {NAV_ITEMS.map((item, index) => (
              <div key={index}>
                {item.children ? (
                  <NavLinkWithDropdown currentPath={currentPath} item={item} />
                ) : (
                  <Link
                    href={item.href}
                    className={`text-gray-900 hover:text-red-500 p-2 hover:bg-gray-100 text-sm ${
                      currentPath === item.href ? "font-bold text-red-600" : ""
                    }`}
                  >
                    {" "}
                    {item.label}
